...
public class MetricsController {
    ...
    @PostMapping("/collect")
    @Operation(
        summary = "Collecter et stocker des métriques", 
        description = "Collecte les métriques pour un type de carte donné et les stocke dans Redis"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<String> collectMetrics(
            @Parameter(description = "Type de carte (VISA, MASTERCARD, AMEX, etc.)", required = true)
            @RequestParam String typeCarte,
            
            @Parameter(description = "Type d'opération (PAYMENT, REFUND, AUTHORIZATION, CAPTURE)", required = true)
            @RequestParam String operationType,
            
            @Parameter(description = "Temps d'exécution en millisecondes", required = true)
    ...
    @Operation(
        ...
        description = "Récupère les métriques stockées pour un type de carte donné"
    )
    @ApiResponses(value = {
    ...
    @GetMapping("/types")
    @Operation(
        summary = "Lister les types de cartes disponibles", 
        description = "Retourne la liste des types de cartes supportés"
    )
    @ApiResponse(responseCode = "200", description = "Liste des types de cartes")
    public ResponseEntity<String[]> getSupportedCardTypes() {
        String[] cardTypes = {"VISA", "MASTERCARD", "AMEX", "DISCOVER", "JCB", "DINERS"};
        return ResponseEntity.ok(cardTypes);
    }

    @GetMapping("/operations")
    @Operation(
        summary = "Lister les types d'opérations disponibles", 
        description = "Retourne la liste des types d'opérations supportés"
    )
    ...
    public ResponseEntity<String[]> getSupportedOperationTypes() {
        String[] operationTypes = {"PAYMENT", "REFUND", "AUTHORIZATION", "CAPTURE", "VOID", "SETTLEMENT"};
        return ResponseEntity.ok(operationTypes);
    }

    @DeleteMapping("/{typeCarte}")
    @Operation(
        summary = "Supprimer les métriques", 
        description = "Supprime les métriques stockées pour un type de carte donné"
    )
    @ApiResponses(value = {
    ...
    @Operation(
        summary = "Collecte de métriques en lot", 
        description = "Collecte plusieurs métriques en une seule requête"
    )
    @ApiResponses(value = {
    ...
    @GetMapping("/health")
    @Operation(
        summary = "Vérification de santé du service de métriques", 
        description = "Vérifie si le service de métriques est opérationnel"
    )
